...
struct sof_ipc4_kcontrol_global_priv {
	...
	struct sof_ipc4_kcontrol_global_capture_hw_mute {
		...
		bool last_reported; /* Last statusmute state received from FW */
	} capture_hw_mute;
};
...
static struct snd_card *sof_scomp_get_card(struct snd_soc_component *scomp)
{
	...
	if (!scomp) {
		dev_err(dev, "%s: scomp == NULL", __func__);
		return NULL;
	}
						 ...
						 struct snd_soc_component *scomp)
{
	...
	if (!ipc4_data) {
		dev_err(dev, "%s: No ipc4_data!", __func__);
		return -ENODEV;
	}
	...
	if (ipc4_data->global_kcontrol_mask &
		...
		if (!kctl) {
			dev_err(dev, "snd_ctl_new1(): failed");
			return -EINVAL;
		}
					 ...
					 struct sof_ipc4_control_msg_payload *msg_data)
{
	...
	dev_info(dev, "%s: got msg id %u num_elems %u", __func__,
